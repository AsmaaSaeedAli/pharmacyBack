...
namespace Pharmacy.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : PharmacyServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("Pharmacy", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
